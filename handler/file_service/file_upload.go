...
func FileUpload(c *gin.Context) {
	var savePath, fileId, userFileId string
	...
	*/
	...
	// 1.处理出文件名、拓展名、文件的逻辑绝对路径、文件类型
	fileInfo := filehandler.GetFileInfoFromReq(req)

	// 2.所有文件分片上传完成，开启mysql事务
	err = models.DB.Transaction(func(tx *gorm.DB) error {
		...
		userFileId = common.GenerateUUID()
		savePath = "./repository/upload_file/" + fileId

		// 将分块文件合并
		err = filehandler.MergeChunksToFile(req.FileMD5, fileId, req.TotalChunks)
		if err != nil {
			return err
		}
	...
	//filehandler.DeleteAllChunks(fileMD5, totalChunks)
	...
	pubMsg := mq.TransferData{
		FildId:        fileId,
		FileHash:      req.FileMD5,
		TempLocation:  savePath,
		DestLocation:  req.FileMD5,
		DestStoreType: "",
		DestType:      0,
	}
	log.Println(pubMsg)
