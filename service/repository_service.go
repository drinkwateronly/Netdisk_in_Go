...
func FileUploadPrepare(c *gin.Context) {
	...
	// 处理出文件名、拓展名、文件类型
	split := strings.Split(fileFullName, ".")
	var fileName, extendName, absolutePath string
	if len(split) == 0 { // 没有后缀
		fileName = split[0]
	} else {
	...
	if filePath == "/" {
		absolutePath = filePath + strings.Trim(relativePath, "/"+fileFullName)
	} else {
		absolutePath = filePath + "/" + strings.Trim(relativePath, "/"+fileFullName)
	}
	...
	// 判断文件在当前文件夹是否重名
	if _, isExist := models.FindFileByNameAndPath(ub.UserId, absolutePath, fileName, extendName); isExist {
	...
	err = utils.DB.Transaction(func(tx *gorm.DB) error {
		...
		if relativePath != fileFullName {
			// 取出 [123, 456]，即文件相对路径先后进入的文件夹的列表
			folderList := strings.Split(relativePath[:len(relativePath)-len(fileFullName)-1], "/")
			// 若文件夹不存在，则创建，若存在，则继续
			for _, folderName := range folderList {
				// 当前文件上传的目录filePath有没有名为folderName的文件夹
				res := utils.DB.Clauses(clause.Locking{Strength: "UPDATE"}).
	...
	if err != nil {
		utils.RespOK(writer, 0, false, gin.H{"skipUpload": true}, "文件上传失败")
	}
...
func FileUpload(c *gin.Context) {
	...
	currentChunkSize, _ := strconv.ParseInt(c.PostForm("currentChunkSize"), 10, 64) // 当前分片的大小，未使用
	...
	fileFullName := c.PostForm("filename")     // 文件名，包括拓展名
	filePath := c.PostForm("filePath")         // 文件在用户存储区的
	isDir := c.PostForm("isDir")               // 是否是文件夹
	relativePath := c.PostForm("relativePath") // 相对路径

	chunkPath := fmt.Sprintf("./repository/chunk_file/%s-%d.chunk", fileMD5, chunkNumber)
	// 该分片可能之前上传过，例如上次上传时失败

	if utils.IsChunkExist(chunkPath, currentChunkSize) {
	...
	// 获取文件基础信息
	split := strings.Split(fileFullName, ".")
	var fileName, extendName string
	if len(split) == 0 { // 没有后缀
		fileName = split[0]
	} else {
		fileName = strings.Join(split[0:len(split)-1], ".")
	...
	if err != nil {
		utils.RespOK(writer, 0, true, nil, "文件上传失败："+err.Error())
		return
	}
...
func FileDownload(c *gin.Context) {
	...
	if os.IsNotExist(err) {
		utils.RespBadReq(writer, "本地文件不存在")
		return
	}
